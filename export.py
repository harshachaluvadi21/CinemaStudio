...
def to_pdf(content: str, section: str, username: str = "User") -> io.BytesIO:
    ...
    # Title Page for ALL sections
    story.append(Paragraph("CINEMA STUDIO PROJECT", title_style))
    story.append(Paragraph(SECTION_TITLES.get(section, section.title()), subtitle_style))
    
    byline = f"Screenplay by {username}" if section == "screenplay" else f"Created by {username}"
    if section == "characters":
        byline = f"Character Profiles by {username}"
    elif section == "sound":
        byline = f"Sound Design by {username}"
        
    story.append(Paragraph(byline, subtitle_style))
